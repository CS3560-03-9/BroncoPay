...
async function createTransaction(body) {
  ...
  const toAccountExists = await accountsController.accountExists(
    body.toHandler
  );
...
async function createDeposit(body) {
  ...
  return await db.query(
    "INSERT INTO `transactions` (`from_handler`, `to_handler`, `amount`, `transaction_desc`, `transaction_type`) VALUES (?, ?, ?, ?, ?)",
    [body.handler, null, body.amount, body.description, "DEPOSIT"]
  );
}
...
async function createWithdraw(body) {
  ...
  return await db.query(
    "INSERT INTO `transactions` (`from_handler`, `to_handler`, `amount`, `transaction_desc`, `transaction_type`) VALUES (?, ?, ?, ?, ?)",
    [body.handler, null, body.amount, body.description, "WITHDRAW"]
  );
}
